...
<body>
	...
	<script type="text/javascript">
		function checkMember() {
			...
			// ([-_.]?[0-9a-zA-Z])* : - 또는 _ 또는 .가 있을 수도 있고 그 뒤에 영문숫자가 와야함 - 이 그룹이 0번 이상 반복
			...
			if (!regExpName.test(name)) {
				alert("이름은 2~4글자의 한글으로만 입력해주세요!");
				return;
			}
			if (!regExpPasswd.test(passwd)) {
				alert("비밀번호는 4자 이상의 숫자만 입력해주세요!");
				return;
			}
